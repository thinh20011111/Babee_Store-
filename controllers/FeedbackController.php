...
class FeedbackController
{
    ...
    public function submitFeedback($request, $files)
    {
        ...
        try {
            ...
            error_log("submitFeedback called with: user_id=$user_id, product_id=$product_id, order_id=$order_id, rating=$rating, content=" . substr($content, 0, 50));
            ...
            $result = $this->feedback->addFeedback($user_id, $product_id, $order_id, $content, $rating);
            if (!$result) {
                throw new Exception('Không thể lưu đánh giá.');
            }

            // Lấy feedback_id từ bản ghi vừa thêm (vì addFeedback không trả về ID)
            ...
            if (!empty($files['images']['name'][0])) {
                ...
                foreach ($files['images']['name'] as $index => $name) {
                    ...
                    $photo_count++;
                    error_log("submitFeedback: Saved media $file_path for feedback_id=$feedback_id");
                }
            }

            $this->db->commit();
            error_log("submitFeedback: Success for feedback_id=$feedback_id");
    ...
    public function updateFeedback($request, $files)
    {
        try {
            if (!isset($_SESSION['user_id'])) {
                error_log("updateFeedback: User not logged in");
                return json_encode(['success' => false, 'message' => 'Vui lòng đăng nhập.']);
            }
            ...
            error_log("updateFeedback called with: user_id=$user_id, feedback_id=$feedback_id, rating=$rating, content=" . substr($content, 0, 50));

            if (!$this->feedback->canEditFeedback($feedback_id, $user_id)) {
                error_log("updateFeedback: No permission for feedback_id=$feedback_id");
                return json_encode(['success' => false, 'message' => 'Bạn không có quyền chỉnh sửa đánh giá này.']);
            }

            if (strlen($content) < 10) {
                error_log("updateFeedback: Content too short");
                return json_encode(['success' => false, 'message' => 'Nội dung đánh giá phải có ít nhất 10 ký tự.']);
            }
            if ($rating < 1 || $rating > 5) {
                error_log("updateFeedback: Invalid rating");
                return json_encode(['success' => false, 'message' => 'Điểm đánh giá phải từ 1 đến 5.']);
            }
            ...
            if (!empty($files['photos']['name'][0])) {
                ...
                foreach ($files['photos']['name'] as $index => $name) {
                    ...
                    $photo_count++;
                    error_log("updateFeedback: Saved media $file_path for feedback_id=$feedback_id");
                }
            }

            $this->db->commit();
            error_log("updateFeedback: Success for feedback_id=$feedback_id");
            return json_encode(['success' => true, 'message' => 'Đánh giá đã được cập nhật thành công!']);
        } catch (Exception $e) {
            if ($this->db->inTransaction()) {
                $this->db->rollBack();
            }
            error_log("updateFeedback Error: " . $e->getMessage() . " at " . $e->getFile() . ':' . $e->getLine());
            return json_encode(['success' => false, 'message' => $e->getMessage()]);
        }
    }
    ...
    public function deleteFeedback($request)
    {
        try {
            if (!isset($_SESSION['user_id'])) {
                error_log("deleteFeedback: User not logged in");
                return json_encode(['success' => false, 'message' => 'Vui lòng đăng nhập.']);
            }
            ...
            error_log("deleteFeedback called with: user_id=$user_id, feedback_id=$feedback_id");

            if (!$this->feedback->canEditFeedback($feedback_id, $user_id)) {
                error_log("deleteFeedback: No permission for feedback_id=$feedback_id");
                return json_encode(['success' => false, 'message' => 'Bạn không có quyền xóa đánh giá này.']);
            }
            ...
            error_log("deleteFeedback: Success for feedback_id=$feedback_id");
            return json_encode(['success' => true, 'message' => 'Đánh giá đã được xóa thành công!']);
        } catch (Exception $e) {
            error_log("deleteFeedback Error: " . $e->getMessage() . " at " . $e->getFile() . ':' . $e->getLine());
            return json_encode(['success' => false, 'message' => $e->getMessage()]);
        }
    }
    ...
    public function getFeedback($product_id)
    {
        try {
            ...
            foreach ($feedbacks as &$feedback) {
                ...
                // Đảm bảo username có giá trị mặc định
                $feedback['username'] = $feedback['username'] ?? 'Khách ẩn danh';
            }
            error_log("getFeedback: Retrieved " . count($feedbacks) . " feedbacks for product_id=$product_id");
            return json_encode(['success' => true, 'data' => $feedbacks]);
        } catch (Exception $e) {
            error_log("getFeedback Error: " . $e->getMessage() . " at " . $e->getFile() . ':' . $e->getLine());
            return json_encode(['success' => false, 'message' => $e->getMessage()]);
        }
    }
}
